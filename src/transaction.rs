...
pub trait Transactional<E=()> {
    ...
    where
        F: Fn(Self::View<'_>) -> ConflictableTransactionResult<A, E>,
		//<Self::View as TransactionalTrees>::Inner: sled::transaction::Transactional<E>
    {
        ...
        sled::transaction::Transactional::<E>::transaction(&self.get_inner(), |sled_view| Self::c(f, sled_view))
    }
    
    fn c<'a, 'b, F, A>(f: F, sled_view: &'a <<Self::View<'b> as TransactionalTrees<E>>::Inner as sled::transaction::Transactional>::View) -> R
    where
        F: Fn(Self::View<'a>) -> ConflictableTransactionResult<A, E>
    {
